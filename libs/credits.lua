...
function SMODS.create_mod_badges(obj, badges)
    ...
    if not SMODS.config.no_mod_badges and obj and obj.ark_credits then
        ...
        if obj.ark_credits.art or obj.ark_credits.code or obj.ark_credits.idea or obj.ark_credits.name or obj.ark_credits.custom then
            local scale_fac = {}
            local min_scale_fac = 1
            local strings = { "Arklatro" }
                        ...
                        nodes = {
                            { n = G.UIT.B, config = { h = 0.1, w = 0.03 } },
                            {
                                ...
                                config = {
                                    object = DynaText({
                                        string = ct or "ERROR",
                                        colours = { obj.ark_credits and obj.ark_credits.text_colour or G.C.WHITE },
                                        silent = true,
                                        float = true,
                                        shadow = true,
                                        offset_y = -0.03,
